...
try {
    ...
    Step "Pinning Python for launchers (python-path.txt)"
    # gitignored — do not rewrite tracked Start-WimsServer.cmd (that blocked Update).
    ...
    if (-not (Test-Path -LiteralPath $startCmd)) {
        throw "Missing tracked launcher: $startCmd"
    }
    Ok $startCmd
