...
- **Per-instance state grid** — one row per WSJT-X: band, mode, freq, RX/TX, current cycle, last
  decode count.
- **QSO exchange monitor (per TX-enabled instance)** — whenever an instance is enabled for
  transmit (click-to-work or seat-armed), show the **message back-and-forth live**: the Tx
  message actually going out this cycle (Status datagram Tx-message field + Transmitting flag),
  the last decode received from the station being worked, the auto-sequence stage
  (calling / report / RR / 73) and a repeat count so a stalled exchange is obvious. The console
  operator covering several bands must be able to supervise every exchange they initiated
  without looking at the seat UI — attended TX in practice, not just at initiation (§Goals
  no-unattended-TX).
